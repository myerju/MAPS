...
log <-  (read_tsv("/Volumes/MAPS1/Participant Recordings/MAPS-005/raw/MAPS-005_log.tsv"))
video_sync <- read.csv("/Users/myerju/Box Sync/MAPS/Data/Sync/sync_timestamps.csv", header = T) #csv of manually recorded timestamps
...
```{r export_csv}
write.csv(log_sync, file = "/Volumes/MAPS1/Participant Recordings/MAPS-005/synced/MAPS-005_log_webcam_sync.csv")
```
...
```{r import_load_packages}
pkgs <- c("tuneR", "signal") # list packages needed
ipak <- function(pkg){
  new.pkg <- pkg[!(pkg %in% installed.packages()[, "Package"])]
  if (length(new.pkg)) 
    install.packages(new.pkg, dependencies = TRUE)
...
```{r import_data}
audio <- readWave("C:/Users/Kevin/Desktop/cryingbeep2.wav", from = 0, to = Inf, units = "minutes") #replace with correct path
```
